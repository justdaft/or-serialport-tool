...
if (process.argv[2] == 'dev') {
  ...
  run(
    `${util.hotEnv} webpack-dev-server --inline --hot --colors --port ${config.port} --content-base app/dist`,
    util.YELLOW, 'webpack');
} else if (process.argv[2] == 'pack') {
  util.colFormat(`Starting webpack...\n`, util.YELLOW);
  run(`${util.packEnv} webpack --progress --colors --hide-modules`, util.YELLOW,
    'webpack');
} else {
